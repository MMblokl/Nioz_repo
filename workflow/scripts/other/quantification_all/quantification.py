...
#all read coverages of all contigs in all bins.
...
#7: The amount of reads mapping to a contig is divided by the length of said contig,
...
#
# (Median reads per contig basepair * estimated bin genomesize) / total adjusted median read amount with estimated genomesize.
...
#
# (Average reads per contig basepair * estimated bin genomesize) / total adjusted average read amount with estimated genomesize.
...
#
# (Median reads per contig basepair / estimated bin genomesize) / total adjusted median read amount with estimated genomesize inversed.


#10: The exact same method as #8 is used, but then we divide by genomesize, not multiply.
#
# (Average reads per contig basepair / estimated bin genomesize) / total adjusted average read amount with estimated genomesize inversed.
...
with open(f"{outputfile}", "w") as o:
  ...
  for bin in bin_quantdata.keys():
    ...
    #Method #10
    adjusted_readratio_avg_i = (bindata["adjusted_read_amt_avg_i"]/gs_i_adjusted_total_avg)*100
